...
#include "tpp/player/appearance/PlayerFaceFovaGateHook.h"
...
static bool __fastcall hkDoesNeedFaceFova(std::uint32_t playerPartsType)
{
    ...
    if (entry && entry->enableHead)
    {
        Log(
            "[FaceFovaGate] Force DoesNeedFaceFova partsType=0x%02X playerType=0x%02X -> true\n",
...
static bool __fastcall hkDoesNeedFaceFovaForAvatar(std::uint32_t playerPartsType)
{
    ...
    if (entry && entry->enableHead && entry->playerType == 3)
    {
        Log(
            "[FaceFovaGate] Force DoesNeedFaceFovaForAvatar partsType=0x%02X playerType=0x%02X -> true\n",
            static_cast<unsigned>(playerPartsType),
            static_cast<unsigned>(entry->playerType)
        );
        return true;
