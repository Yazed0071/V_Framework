    ...
    std::uint32_t suitCode)
{
    ...
    if (pendingDevelopId != 0 && pendingDevelopId != 0xFFFF)
    {
        if (code == 0xFF)
        {
            ...
            {
                Log(
                    "[SuitResolver] pending developId=%u playerType=%u incomingSelector=0x%02X -> partsType=0x%02X selector=0x%02X\n",
                    ...
                    static_cast<unsigned>(pendingEntry->customSelectorCode)
                );
