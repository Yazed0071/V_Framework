...
namespace outfit::shadow
{
    void Set(std::size_t slot, const Slot& s)
    {
        ...
        // UpdatePartsStatus calls Set every tick (~60 Hz) for active slots, so
        // log only on actual changes — otherwise the log floods with identical
        // entries. The first SET on a previously-empty slot still logs because
        // `used` flips from false → true.
